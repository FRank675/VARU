...
class lstm:
    def __init__(self, layers, n_units=45):
        self.model = Sequential()
        self.model.add(Dropout(0.1))
        self.model.add(Bidirectional(LSTM(n_units, input_shape=(layers[1], layers[0]), return_sequences=False)))
        self.model.add(Dropout(0.1))
    ...
    def fit(self, X, y):
        g = DataGenerator(X, y)
        history = self.model.fit_generator(g.generate(), X.shape[0]//(g.batch_size*g.num_steps), epochs=32, verbose=True)

    def predict(self, X):
        return self.model.predict(X)

    def saveLstm(self):
        self.model.save('lstm-a.model')
